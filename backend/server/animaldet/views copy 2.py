...
# Settings
# ---------------------------------------------------------------------------
WEIGHTS         = r"/home/peter/projects/MajorProject/Msjor_project_final/AI-based-Animal-Monitoring-and-Behaviour-Analysis/videoScript/yolo_v26_best.pt"
...
def analyze_activity_from_queue(state: CameraState):
    ...
    try:
        ...
        for idx, frame in enumerate(sampled_frames):
            try:
                ...
                if results and results[0].boxes is not None:
                    ...
                    for c in clsi:
                        detected_species.add(model.names[c])
            except Exception as e:
        ...
        species_str = ", ".join(sorted(detected_species)) if detected_species else "wildlife"
        prompt_text = (
...
def capture_frames(state: CameraState):
    try:
        ...
        while state.stream_running:
            ...
            annotated = results[0].plot() if results and hasattr(results[0], "plot") else frame.copy()
...
def _get_last_10_animals():
    """Synchronous helper to query the last 10 animal records."""
    recent = Animal.objects.all().order_by('-timestamp')[:10]
    return [
        {
            "id": a.id,
            "species": a.species,
...
async def sse_live_stats():
    try:
        while True:
            ...
            # Fetch last 10 detections directly from DB
            # Flow: Detection -> DB write -> SSE query -> Frontend
            try:
                stats["recent_history"] = await sync_to_async(_get_last_10_animals, thread_sensitive=True)()
            except Exception as e:

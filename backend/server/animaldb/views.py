...
class AnimalViewSet(viewsets.ModelViewSet):
    ...
    def yearly_summary(self, request):
        ...
        queryset = Animal.objects.filter(timestamp__year=year)
        ...
        # ✅ correct total count using event logic
        total_animals = self._calculate_event_count(queryset)

        # ✅ max individuals spotted in one event
        TIME_WINDOW = timedelta(minutes=2)
        ...
        for r in records:
            ...
            for o in records:
                if (
                    o.id not in processed
                    and o.camera_id == r.camera_id
                    and r.timestamp <= o.timestamp < r.timestamp + TIME_WINDOW
                ):
                    event.append(o)
                    processed.add(o.id)

            max_event = max(x.count for x in event)
            max_individuals = max(max_individuals, max_event)

        # ✅ favourite activity (correct)
        favourite_activity = (
            queryset.values("behaviour")
            .annotate(c=Count("id"))
            .order_by("-c")
            .first()
        )

        fav = favourite_activity["behaviour"] if favourite_activity else "N/A"

        # ✅ top locations using event logic
        locations = queryset.values("latitude", "longitude").distinct()
        ...
        for loc in locations:
            ...
            loc_qs = queryset.filter(latitude=lat, longitude=lon)

            cnt = self._calculate_event_count(loc_qs)
        ...
        top_locations = sorted(top_locations, key=lambda x: x["count"], reverse=True)[:3]

        return Response({
            "year": year,
            "total_animals": total_animals,
            "max_individuals_spotted": max_individuals,
            "favourite_activity": fav,
            "top_3_most_visited": top_locations
        })
